...
E:\javacode\xingang-community-deepseek    feature/deepseek-business-phase2，阶段二已合并；后续任务需新开分支
E:\javacode\xingang-community-copilot     feature/copilot-ai-phase2，阶段二已合并；后续任务需新开分支
```
...
1. 产品经理 AI 先和用户确认第三阶段目标：演示体验、测试补齐、真实接口闭环、自然语言回答优化，或继续扩展业务功能。
2. 根据目标重新开两个 feature 分支，不再继续在 `feature/deepseek-business-phase2` 和 `feature/copilot-ai-phase2` 上开发。
3. DeepSeek 可优先处理：死信队列、Redis Stream 监控、库存重置管理接口、业务接口联调测试。
4. Copilot 可优先处理：将 Tool 事实组织成自然语言推荐回答、上下文指代解析、推荐解释、演示 prompt 和接口示例。
5. 每个执行者正式编码前仍必须先输出复杂功能实现方案，等待确认后再动手。
